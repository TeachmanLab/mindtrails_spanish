...
def create_input(tipe, items=None, min=None, max=None, text=None):
    ...
    if tipe == "buttons"  : return {"type": "Buttons", "buttons": items, **({"ColumnCount": 2} if is_yesno(items) else {}) }
    ...
    if tipe == "checkbox" : return {"type": "Buttons", "buttons": items, "multiselect": True }
