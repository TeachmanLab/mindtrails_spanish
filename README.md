...
The protocol can be downloaded in the Digital Trails app using the study code, "MTS" on the Apps initial startup screen.
...
## Updating The Protocol's JSON

All of the studie's custom content are described by JSON files. To recreate these files:
1. Run `make/scripts/microdoses.py`, `make/scripts/surveys.py`, and `make/scripts/resources.py`.
2. Copy the output from *Step 1*. The output will be folders in the `make/~out/` directory.
3. Delete the folders in `src/flows/` (*note*: you should only delete the folders). The files are not generated by make scripts.
4. Paste the output put copied in *Step 2*

## Release Updates To The Protocol
...
## Releasing Multiple Protocol Versions
